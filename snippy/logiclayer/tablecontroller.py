...
class TableController:
    ...
    def create_table(self, clobber=False):
        if clobber:
            ...
            logging.info("Dropped table {0}".format(self._table.name))
        sql = self._sql_gen.get_create_table_sql()
        Sqlite.execute_sql(self._db_conn, sql)
        logging.info("Created table {0}".format(self._table.name))
